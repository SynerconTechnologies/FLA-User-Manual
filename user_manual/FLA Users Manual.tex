...
It is suggested operators examine this document entirely before using
...
\section{The FLA Preview}\label{sec:fla_preview}
\vspace{1cm}
\paragraph{  }
...
\ref{sec:fla_portal} and \ref{sec:fla_preview}.
...
\paragraph{  }
Enabling DHCP allows the FLA to communicate with computers when a network is not present. I.E. to preview data in the field. Do not enable DHCP services while the FLA is plugged into an active network.
\\[\baselineskip]
\noindent\begin{minipage}{0.45\textwidth}% adapt widths of minipages to your needs
\begin{center}
...
\paragraph{  }
Dissabling DHCP allows the FLA to communicate with an active network. DHCP is disabled when the FLA boots.
\\[\baselineskip]
\noindent\begin{minipage}{0.45\textwidth}% adapt widths of minipages to your needs
\begin{center}
\textbf{1}\\[\baselineskip]
...
\paragraph{  }
When the FLA is being used in the field, the DHCP services may be tuned on. These services allow the user to connect a computer directly to the FLA using the provided Ethernet connection. For more information on enabling DHCP services, refer to section \ref{subsec:enable_dhcp}.
\begin{center}
...
in a supported web browser. For a list of supported web browsers, refer to section \ref{sec:fla_portal}
\paragraph{  }
...
\section{Downloading ECMs with OEM Software}
\paragraph{  }
...
\begin{figure}
\includegraphics[width=.9\textwidth]{../media/ddec_screenshots/rp1210_ddec_reports_download_page}

\caption{\label{fig:Download-page-forDDEC}Download page for DDEC Reports.
The link to download is at the bottom of the page. The link to this
page is on the left menu for DiagnosticLink 7.x/8.x and is indicated
with a loop. }
...
\item Once the data is extracted, Select File -> Print and print all the
...
\item View and Diagnostics -> Fault Codes then File -> Print Fault Codes\\
...
\item Select Diagnostics -> ECM I/O Configuration. This can be printed and
...
\item Select Diagnostics -> Injector Response Times to print.\\
\includegraphics{../media/dddl_screenshots/dddl6_injector_response_times}
\item Select Diagnostics -> Maintenance Alert\\
\includegraphics{../media/dddl_screenshots/dddl6_maintenance_alert}
\item Select Tools -> Engine/Trip Data\\
\includegraphics{../media/dddl_screenshots/dddl6_engine_and_trip_data}
\item Take a screen shot of the software verison number from Help -> About\\
...
\section{Caterpillar ET Download Protocol}
...
procedure for obtaining and installing these drivers is found in Section
\ref{sub:RP1210-Drivers}.
...
\begin{figure}[h]
\includegraphics[width=.8\textwidth]{../media/fla_portal_screenshots/FLAPortalFieldOperatorsScreenShot}
...
\begin{figure}[h]
\includegraphics[height=.6\linewidth]{../media/fla_portal_screenshots/FLAPortalInviteNewOperatorScreenShot}
...
\section{Performing a download on a Heavy Vehicle}
